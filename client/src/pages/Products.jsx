          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-2xl shadow-2xl p-6 space-y-4 max-h-[85vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveProduct} className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
              ...
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Product Code / Style Code (e.g., D0001)</label>
                <input
                  type="text"
                  value={formCode}
              ...
              {/* Barcode Value Input */}
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Barcode Value (optional)</label>
                <input
                  type="text"
            ...
            <div className="border border-slate-100 dark:border-slate-800 p-4 rounded-xl bg-slate-100 dark:bg-slate-950/80 max-h-80 overflow-y-auto flex justify-center">
              <div className={`grid gap-2 bg-white p-3 shadow-inner rounded border ${printCopies === 1 ? 'grid-cols-1' : 'grid-cols-4'}`}>
                {Array.from({ length: printCopies }).map((_, i) => (
                  <div 
                    key={i} 
                    className={`border border-slate-400/60 rounded text-center bg-white text-black flex flex-col justify-between items-center shadow-sm ${
                      printCopies === 1 ? 'w-48 h-28 p-2.5 space-y-1.5' : 'w-[105px] h-[72px] p-1.5 space-y-0.5'
                    }`}
                  >
                    <p className={`font-extrabold uppercase leading-none text-slate-900 ${
                      printCopies === 1 ? 'text-[10px] tracking-widest' : 'text-[7px] tracking-wide'
                    }`}>
                      {barcodePrintData.shopName}
                    </p>
                    <p className={`font-bold leading-tight truncate w-full ${
                      printCopies === 1 ? 'text-[9px] text-slate-800' : 'text-[7px] text-slate-700'
                    }`}>
