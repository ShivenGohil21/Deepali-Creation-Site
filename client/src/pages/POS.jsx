...
const POS = () => {
  ...
  const handleBarcodeScanSubmit = async (e) => {
    ...
    try {
      ...
      // Auto-normalize code formatting, e.g. d003 -> d0003, d3 -> d0003
      const normalizedVal = (() => {
    ...
    <>
      ...
      {toast.text && (
        <div className={`fixed bottom-4 right-4 px-4 py-3 rounded-xl shadow-lg z-50 flex items-center space-x-2 text-xs font-semibold ${
          toast.type === 'error' ? 'bg-red-500 text-white' : 'bg-primary-600 text-white'
        }`}>
      ...
      <div className="lg:col-span-5 flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-4 overflow-hidden h-full shadow-sm">
        ...
        <div className="border-t border-slate-100 dark:border-slate-800 pt-3 space-y-2 text-xs">
          ...
          <div className="bg-slate-50 dark:bg-slate-950/40 p-3 rounded-xl space-y-1.5">
            ...
            <div className="flex justify-between">
              <span className="text-slate-400">Tax Amount ({tax}%):</span>
              <span className="font-semibold text-slate-700 dark:text-slate-300">₹{totals.taxAmount.toFixed(2)}</span>
            </div>
            <div className="flex justify-between">
            ...
            gridFilteredProducts.map((p) => {
              ...
              return (
                <button
                  ...
                  className={`bg-slate-50 dark:bg-slate-950 border rounded-2xl p-3.5 text-left flex flex-col justify-between hover-scale shadow-sm text-xs ${
                    qty <= 0 
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      className={`py-3.5 rounded-xl border flex flex-col items-center justify-center font-bold space-y-1.5 transition ${
        ...
        <div className="hidden print-area">
          <div className="p-6 bg-white text-black font-mono text-[12px] max-w-[3in] mx-auto space-y-3 leading-relaxed">
            ...
            <div className="space-y-1 text-right text-[11px]">
              ...
              {checkoutInvoice.tax > 0 && <div>Tax ({checkoutInvoice.tax}%): ₹{((checkoutInvoice.subTotal * checkoutInvoice.tax)/100).toFixed(2)}</div>}
